        ...
        while True:
            ...
            if not next_offset:
                authlogs = await self.loop.run_in_executor(self._executor,
                                                       functools.partial(self.admin_api.get_authentication_log, api_version=2, mintime=mintime))
            else:
                authlogs = await self.loop.run_in_executor(self._executor,
                                                       functools.partial(self.admin_api.get_authentication_log, api_version=2, mintime=mintime, next_offset=next_offset))
